...
segments_fp = os.path.join(args.out_data_dir, 'segments')
utt2spk_fp = os.path.join(args.out_data_dir, 'utt2spk')
wav_scp_fp = os.path.join(args.out_data_dir, 'wav.scp')

# "-R" forces the default random seed for reproducibility
resample_cmd = "sox -q -R -t wav - -t wav - rate {}".format(args.rate)

with open(segments_fp, 'w', buffering=1) as segments_f, open(utt2spk_fp, 'w', buffering=1) as utt2spk_f, open(wav_scp_fp, 'w') as wav_scp_f:
    for line in open(args.script):
        ...
        for speaker in indata['speakers']:
            ...
            for interval, utt in zip(intervals, utts):
                ...
                segments_f.write('%s %s %0.07d %0.07d\n' % (uttid, recid, startpos / args.rate, endpos / args.rate))
                utt2spk_f.write('%s %s\n' % (uttid, spkid))
