...
data_root="/export/common/data/corpora/LDC"
...
if [ $begin_stage -le 0 ] && [ $end_stage -ge 0 ]; then
    ...
    if ! validate_data_dir.sh --no-text --no-feats "${exp_root}/data/callhome1_spk2" \
        ...
        for dset in callhome1 callhome2; do
            ...
            utils/copy_data_dir.sh "${exp_root}/data/${dset}" "${exp_root}/data/${dset}_spk2"
            utils/filter_scp.pl <(awk '{if($2==2) print;}'  "${exp_root}/data/${dset}/reco2num_spk") \
                "${exp_root}/data/${dset}/wav.scp" > "${exp_root}/data/${dset}_spk2/wav.scp"
            ...
            awk '{printf "%s_%s_%07d_%07d %s %.2f %.2f\n", \
                ...
                "${exp_root}/data/callhome/fullref.rttm" | sort > "${exp_root}/data/${dset}_spk2/segments"
            utils/fix_data_dir.sh "${exp_root}/data/${dset}_spk2"
            # Speaker ID is '[recid]_[speakerid]
            awk '{split($1,A,"_"); printf "%s %s_%s\n", $1, A[1], A[2]}' \
                "${exp_root}/data/${dset}_spk2/segments" > "${exp_root}/data/${dset}_spk2/utt2spk"
            utils/fix_data_dir.sh "${exp_root}/data/${dset}_spk2"
            # Generate rttm files for scoring
            steps/segmentation/convert_utt2spk_and_segments_to_rttm.py \
                "${exp_root}/data/${dset}_spk2/utt2spk" "${exp_root}/data/${dset}_spk2/segments" \
                "${exp_root}/data/${dset}_spk2/rttm"
    ...
    if ! validate_data_dir.sh --no-text --no-feats ${exp_root}/data/swb_sre_comb; then
        ...
        local/make_swbd2_phase3.pl $data_root/LDC2002S06 \
            ${exp_root}/data/swbd2_phase3_train
        local/make_swbd_cellular1.pl $data_root/LDC2001S13 \
            ${exp_root}/data/swbd_cellular1_train
    ...
    if ! validate_data_dir.sh --no-text --no-feats ${exp_root}/data/swb_sre_cv; then
        ...
        fix_data_dir.sh ${exp_root}/data/swb_sre_comb_seg
        utils/subset_data_dir_tr_cv.sh ${exp_root}/data/swb_sre_comb_seg ${exp_root}/data/swb_sre_tr ${exp_root}/data/swb_sre_cv
    fi
fi
...
if [ $begin_stage -le 2 ] && [ $end_stage -ge 2 ]; then
    ...
    adapt_set=${exp_root}/data/eval/callhome1_spk2
